...
func BenchmarkLuminaConcurrent(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		Lumina1 := ti.NewLTree[string](c.NewTCPProvider(2, port1))
		port2, err := GetFreePort()
		if err != nil {
			b.Fatal(err)
		}
		Lumina2 := ti.NewLTree[string](c.NewTCPProvider(2, port2))
		port3, err := GetFreePort()
		if err != nil {
			b.Fatal(err)
		}
		Lumina3 := ti.NewLTree[string](c.NewTCPProvider(2, port3))
...
func BenchmarkLuminaSequential(b *testing.B) {
	log.SetOutput(ioutil.Discard)
	for i := 0; i < b.N; i++ {
		port1, err := GetFreePort()
		if err != nil {
		...
		children, _ := Lumina1.GetChildren(Lumina1.Root())
		if time.Since(startTime) >= 10*time.Second {
			b.Fatalf("Timeout reached: %v", len(children))
		}
	}
}
