...
Vector clocks are another logical clock that provide more gurantees than lamport clocks. They are similar to lamport clocks, but instead of having a single counter, they have a counter for each node.
When an event happens on a node, the node increments its own counter, and then sends the vector timestamp to the other nodes. When a node receives a vector timestamp, it updates its own vector timestamp to be the maximum of its own vector timestamp and the received vector timestamp \cite{mattern1989virtual}.\par

Vector timestamps can be ordered by the following rules:
...
This can be used to create a partial order, as concurrent events are not ordered.
