...
Lamport clocks were created by Lamport in 1978\cite{lamport1978time}. They are a logical clock which can be used to track the happened-before relation between events. They work by having each node maintain their own counter, which is incremented on each local event and attached to that event. This attached counter value is called a lamport timestamp. When nodes send messages to each other, they send their current counter value along with it. Then, the receiving node updates their counter to the received lamport timestamp if it is greater than their current counter value. It is important to note that sending and receiving messages both count as events. \par

With lamport clocks, it gives us the guarantee that if $a$ $\rightarrow$ $b$, then $L(a) < L(b)$. However, the reverse is not true. This can be denoted as: 
\[a \rightarrow b \implies  L(a) < L(b)\]
This is useful for detecting potential causal dependencies, however, by using lamport clocks, concurrent events may appear to be causally dependent. One use for lamport clocks is to create a total order which captures the causal dependencies. This can be achieved by assigning each node a unique ID, and using that to break ties when $L(a) = L(b)$. This requires the corresponding node ID to be stored in the lamport timestamp for each event. It is worth noting that this total order is arbitrary, but deterministic. This was defined by Lamport\cite{lamport1978time}, and can be denoted as:\par
