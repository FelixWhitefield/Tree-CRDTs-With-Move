...
\subsection{Lumina}
In Lumina, a total order (priority in Maram) is given to move operations which is used to resolve conflicts between concurrent move operations. Whereas Maram will only apply conflict resolution when there is a conflict, Lumina will always apply conflict resolution for move operations. This does not produce a large overhead, and may even be faster, as Maram still has to check all concurrent move operations for conflicts. Undo is a fast operation, and redo has similar overhead to determining if there is a conflict in Maram. \par
