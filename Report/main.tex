...
Operation-based CRDTs, or otherwise called Commutative Replicated Data Types (CmRDTs), only send the update operation to other replicas. Each operation or update is a pair consisting of a \textit{prepare} method and a \textit{effect} method, where the \textit{prepare} method is only executed at the local replica\cite{10.1007/978-3-642-24550-3_29}. The operations also have to be commutative, but not idempotent. Because of the lack of idempotence, there are more requirements on transmission between replicas as they have to ensure each operation is sent to each replica only once (This can be seen in Figure \ref{fig:operation}). \par
As Operation-based CRDTs have a weak definition, pure operation-based CRDTs have been proposed\cite{10.1007/978-3-662-43352-2_11}. These impose restrictions on the \textit{prepare} method, requiring that it does not inspect that state and can only return the operation. They also require the state of the object to be comprised of a partially ordered log of operations and therefore they take up less space than normal operation-based CRDTs.
...
\begin{table}[ht]
    ...
    \begin{tabular}{c c c} % centered columns (4 columns)
        ...
        Delivery Layer & \makecell{Eventual Consistency                                                                                          \\ \textit{Low Cost}} & \makecell{Causal Consistency \\ \textit{High Cost}}  \\ % inserting body of the table
        Operations     & \makecell{Total Order                                                                                                   \\ \textit{High Cost}} & \makecell{Partial Order \\ \textit{Low Cost}}
