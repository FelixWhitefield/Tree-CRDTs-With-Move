...
\begin{figure}[h]
    \centering
    \fbox{\includegraphics[width=12cm]{clocks.jpeg}}
...
\subsubsection{CRDT and Interface}
Since the CRDT layer is the primary focus of this paper, it will comprise multiple components, making it the largest layer. The components will be specific to each CRDT, and will be implemented using the algorithms described in Section \ref{sssec:algorithms}. \par

\begin{figure}[h]
    \centering
    \fbox{\includegraphics[width=12cm]{crdt.jpeg}}
...
The classes that implement \texttt{Tree} from Figure \ref{fig:crdt} will also be responsible for sending operations to the network layer to be broadcast to other peers. As well as receiving operations from the network layer, and either applying them to the local CRDT or buffering them, depending on the consistency model. \par
...
This helps to simplify the design, as the network layer does not need to know about the CRDTs, and the CRDTs do not need to know about the network layer. \par 

\begin{figure}[h]
    \centering
    \fbox{\includegraphics[width=6cm]{connection.jpeg}}
