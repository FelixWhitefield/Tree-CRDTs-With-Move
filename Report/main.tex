...
\begin{titlepage} % Suppresses displaying the page number on the title page and the subsequent page counts as page 1
    ...
    {\huge\bfseries An Implementation of Tree CRDTs With Highly Available Move Operations}\\[0.4cm] % Title of your document
...
Conflict-Free Data Types (CRDTs) are a class of data structures which allow for concurrent updates without the need for synchronisation. This allows for highly available systems to be created, which are able to operate in the presence of network partitions. Creating a tree CRDT which supports highly available move operations is difficult, as trees have a strict invariant which has to be kept. A move operation can move a node under itself or a descendant of itself, which would create a cycle in the tree and therefore an incorrect state. Because of this, the CRDT algorithm needs a way to detect and resolve cycles. 

This project implements two tree CRDT algorithms which have move operations, one of which was created during this project. This report evaluates their performance and correctness. The implementations are provided for other developers to use in their own applications or to provide a basis for further research. The performance testing results show that the algorithms exhibit a high efficiency in managing a substantial number of concurrent operations. They deliver an impressive throughput of approximately 10,000 operations per second.
...
\subsection{After}
The after tests are similar to the concurrent tests, except that the peers connect to each other and share operations after each peer has applied $n$ operations locally. The results are shown in Figure \ref{fig:after_test}. The x-axis on this graph is $10^3$ instead of $10^4$ because the after tests took much longer than the concurrent and sequential tests. This gives insight into the overhead that can occur when local replicas make progress locally without communicating with other peers.
...
%Time taken for implementation vs expected
