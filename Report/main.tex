...
Maram \cite{https://doi.org/10.48550/arxiv.2103.04828} was deemed too complex to implement, but during the process of attempting implementation, a potentially superior algorithm was discovered. The new algorithm was inspired by Maram and Kleppmann's algorithm, and will be referred to as \textit{Lumina}. Lumina uses the guarantees provided by a causal delivery layer from Maram, and the undoing, and redoing of operations from Kleppmann's algorithm. With a causal delivery layer, it allows the algorithm to relax which operations are logged, and need to be undone and redone. \par 

A total order is given to move operations, which is used to resolve conflicts between concurrent move operations. This is similar to the priority given to move operations in Maram. Whereas Maram will only apply conflict resolution when there is a conflict, Lumina will always apply conflict resolution. This does not produce a large overhead, and may even be faster, as Maram still has to check all concurrent move operations to see if there is a conflict. Undo is a fast operation, and redo has similar overhead to determining if there is a conflict in Maram. \par
...
\begin{figure}[h]
    \dirtree{%
        ...
        .4 test.\DTcomment{Contains the tests for the clock package}.
        .3 connection.\DTcomment{Contains the connection package}.
        .4 message.pb.go.\DTcomment{Contains the generated protocol buffer file}.
        .4 message.proto.\DTcomment{Contains the protocol buffer file}.
        .3 treecrdt\DTcomment{Contains the treecrdt package}.
        .4 kleppmann\DTcomment{Contains Kleppmann's algorithm}.
        .4 maram\DTcomment{Contains the Maram algorithm}.
        .3 treeinterface\DTcomment{Contains the treeinterface package}.
        .3 go.mod\DTcomment{Contains the go module file}.
        .3 go.sum\DTcomment{Contains the go module sum file}.
